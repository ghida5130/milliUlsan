...
function populateMarkers(map, bounds, isRefresh) {
    return fetch("https://api.waqi.info/v2/map/bounds/?latlng=" + bounds + "&token=" + token())
        .then((x) => x.json())
        .then((stations) => {
            ...
            return stations.data.map((station) => new L.LatLng(station.lat, station.lon));
        })
        .catch((e) => {
...
function populateAndFitMarkers(map, bounds) {
    ...
    populateMarkers(map, bounds).then((markerBounds) => {
        ...
        let mapBounds = L.latLngBounds(L.latLng(lat2, lng2), L.latLng(lat1, lng1));
...
function getMarkerPopup(markerUID) {
    return getMarkerAQI(markerUID).then((marker) => {
        ...
        info += marker.attributions
            .map((attribution) => "<a target=_ href='" + attribution.url + "'>" + attribution.name + "</a>")
            .join(" - ");
        return info;
    });
}
...
function getMarkerAQI(markerUID) {
    return fetch("https://api.waqi.info/feed/@" + markerUID + "/?token=" + token())
        .then((x) => x.json())
        .then((data) => {
